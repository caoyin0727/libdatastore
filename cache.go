...
func (c *cache) kmap(kvObject KVObject) (kvMap, error) {
	...
	c.Lock()
	keyPrefix := c.ds.key(kvObject.KeyPrefix()...)
...
func (c *cache) add(kvObject KVObject) error {
	...
	c.Lock()
	kmap[c.ds.key(kvObject.Key()...)] = kvObject
	c.Unlock()
	return nil
}
...
func (c *cache) del(kvObject KVObject) error {
	...
	c.Lock()
	delete(kmap, c.ds.key(kvObject.Key()...))
	c.Unlock()
	return nil
}
...
func (c *cache) get(key string, kvObject KVObject) error {
	...
	o, ok := kmap[c.ds.key(kvObject.Key()...)]
	if !ok {
