...
func newClient(scope string, kv string, addr string, config *store.Config, cached bool) (DataStore, error) {
	...
	if kv == string(store.BOLTDB) {
		...
		if len(parts) == 2 {
			rootchains = strings.Split(parts[1], "/")
		}
	}
	...
	ds := &datastore{scope: scope, store: store, active: true, watchCh: make(chan struct{}), rootchains: rootchains}
	if cached {
...
func (ds *datastore) key(key ...string) string {
	keychain := append(ds.rootchains, key...)
...
func (ds *datastore) Watch(kvObject KVObject, stopCh <-chan struct{}) (<-chan KVObject, error) {
	...
	kvpCh, err := ds.store.Watch(ds.key(kvObject.Key()...), sCh)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		...
		kvpCh, err = ds.store.Watch(ds.key(kvObject.Key()...), sCh)
		if err != nil {
			log.Printf("Could not watch the key %s in store: %v", ds.key(kvObject.Key()...), err)
		}
...
func (ds *datastore) PutObjectAtomic(kvObject KVObject) error {
	...
	if kvObject == nil {
		return fmt.Errorf("invalid KV Object : nil")
	}

	kvObjValue := kvObject.Value()

	if kvObjValue == nil {
		return fmt.Errorf("invalid KV Object with a nil Value for key %s", ds.key(kvObject.Key()...))
	}
	...
	if kvObject.Exists() {
		previous = &store.KVPair{Key: ds.key(kvObject.Key()...), LastIndex: kvObject.Index()}
	} else {
		previous = nil
	}

	_, pair, err = ds.store.AtomicPut(ds.key(kvObject.Key()...), kvObjValue, previous, nil)
	if err != nil {
		return err
	}
...
func (ds *datastore) putObjectWithKey(kvObject KVObject, key ...string) error {
	...
	return ds.store.Put(ds.key(key...), kvObjValue, nil)
}
...
func (ds *datastore) DeleteObject(kvObject KVObject) error {
	...
	return ds.store.Delete(ds.key(kvObject.Key()...))
}
...
func (ds *datastore) DeleteObjectAtomic(kvObject KVObject) error {
	...
	if _, err := ds.store.AtomicDelete(ds.key(kvObject.Key()...), previous); err != nil {
		return err
	}
...
func (ds *datastore) DeleteTree(kvObject KVObject) error {
	...
	return ds.store.DeleteTree(ds.key(kvObject.KeyPrefix()...))
}
